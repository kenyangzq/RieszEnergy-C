...
//declaring all necessary variables to run gradient descent
  ...
  cube* neighborhood = malloc(pow(3, dim) * sizeof(cube)); // the cubes around the target cubes
  ...
  int* testIndex = malloc(dim * sizeof(int)); // for debugging ?
  int* neighborArray = malloc(maxNbrs*numpts*sizeof(int)); // maxNbrs maximum number of points in nbhd
